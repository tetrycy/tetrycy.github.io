...
function renderScoreboard() {
  ...
  document.getElementById('po-scoreboard-me').innerHTML = buildLines(state.liveScorersMe || [], state.liveCardsMe || [], state.livePenaltyMissesMe);
  document.getElementById('po-scoreboard-opp').innerHTML = buildLines(state.liveScorersOpp || [], state.liveCardsOpp || [], state.livePenaltyMissesOpp);
}
...
function resetMatchScreen(opp, myTeam, venue) {
  ...
  if (typeof applyTacticsPreset === 'function') applyTacticsPreset();
  ...
  const seasonNextBtn = document.getElementById('season-btn-next-round');
  if (seasonNextBtn) seasonNextBtn.style.display = 'none';
  const challengeNextBtn = document.getElementById('challenge-btn-next-round');
  if (challengeNextBtn) challengeNextBtn.style.display = 'none';
  ...
  state.liveMatchRoster = activeIds.map(id => {
    ...
    return buildMatchRosterEntry(p, id);
  });
  ...
  state.livePenaltyMissesMe = []; state.livePenaltyMissesOpp = [];
  ...
  const myLDef = myL.DEF + venueResult.myBonus.DEF, myLMid = myL.MID + venueResult.myBonus.MID, myLFwd = myL.FWD + venueResult.myBonus.FWD;
  const oppLDef = oppL.DEF + venueResult.oppBonus.DEF, oppLMid = oppL.MID + venueResult.oppBonus.MID, oppLFwd = oppL.FWD + venueResult.oppBonus.FWD;
  document.getElementById('po-my-lines').textContent = `OBR ${myLDef.toFixed(1)} · POM ${myLMid.toFixed(1)} · ATK ${myLFwd.toFixed(1)}`;
  document.getElementById('po-opp-lines').textContent = `OBR ${oppLDef.toFixed(1)} · POM ${oppLMid.toFixed(1)} · ATK ${oppLFwd.toFixed(1)}`;
  // OVR ogólny = po prostu średnia z tych samych trzech linii, policzonych wyżej —
  // NIGDY osobna, zapamiętana wcześniej liczba (state.myOverall / opp.overall),
  // która mogła się zdezaktualizować (np. przez trwały rozwój z treningu poziomu 2
  // między meczami) i pokazywać co innego niż faktyczny, aktualny skład.
  document.getElementById('po-my-ovr').textContent = `OVR ${Math.round((myLDef + myLMid + myLFwd) / 3)}`;
  document.getElementById('po-opp-ovr').textContent = `OVR ${Math.round((oppLDef + oppLMid + oppLFwd) / 3)}`;
  // Jedyne miejsce, które przenosi grę do zamkniętego środowiska meczu —
  // każdy tryb gry (playoff/mundial/sezon/wyzwanie/kreator turnieju) wywołuje
  // tylko resetMatchScreen(), bez własnego showScreen('screen-playoff'/...).
...
const MATCH_TOGGLE_PANEL_IDS = ['lineups-panel', 'commentary-options-details', 'tactics-details'];

// Wspólna logika dla SKŁADY / ⚙ / TAKTYKA — na ekranie meczu tylko JEDEN z
// tych trzech paneli może być otwarty naraz. Otwarcie jednego zamyka
// pozostałe dwa, zamiast się na sobie piętrzyć.
function toggleMatchPanel(panelId) {
  const panel = document.getElementById(panelId);
  const willShow = panel.style.display === 'none';
  MATCH_TOGGLE_PANEL_IDS.forEach(id => { document.getElementById(id).style.display = 'none'; });
  panel.style.display = willShow ? 'block' : 'none';
  return willShow;
}

function toggleLineups() {
  toggleMatchPanel('lineups-panel');
}

function toggleCommentaryOptionsPanel() {
  toggleMatchPanel('commentary-options-details');
}
...
function toggleTacticsPanel() {
  const willShow = toggleMatchPanel('tactics-details');
...
function confirmMatchSub(activeId) {
  ...
  if (state.liveMatchRoster) {
    ...
    if (idx !== -1) {
      state.liveMatchRoster[idx] = applyTrainingDeltasToRoster([buildMatchRosterEntry(reservePlayer, reserveId)])[0];
    }
  }
...
function confirmLineupSwap(activeId) {
  ...
  if (!state.startingXI.includes(activeId)) {
    // ZABEZPIECZENIE: jeśli activeId nie jest już w jedenastce (np. przestarzały
    // widok po innej zmianie), samo dopisanie rezerwowego bez kogoś usuwanego
    // powiększyłoby jedenastkę ponad 11 — to był realny sposób, w jaki skład
    // rósł do 12/13 zawodników. Zamiast tego przerywamy i każemy odświeżyć widok.
    alert('Ten zawodnik nie jest już w jedenastce — odśwież widok i spróbuj ponownie.');
    state.swapPendingReserveId = null;
    renderManageSquad();
    return;
  }
  ...
  state.startingXI = state.startingXI.filter(id => id !== activeId && id !== reserveId).concat([reserveId]);
  state.benchIds = state.benchIds.filter(id => id !== reserveId && id !== activeId).concat([activeId]);
  // isReserve na samych zawodnikach musi zostać w zgodzie z tymi tablicami —
  // inaczej ensureStartingXI() (który w razie awarii odbudowuje jedenastkę
  // z isReserve, nie z tych tablic) mógłby po cichu cofnąć całą zamianę.
  reservePlayer.isReserve = false;
  activePlayer.isReserve = true;
...
function playMatch() {
  ...
  const initialTactics = readTactics();
  initialTactics.oppCoachTraits = (typeof getTeamCoachTraits === 'function') ? getTeamCoachTraits(opp) : [];
  state.liveGen = simulateMatchV2Live(state.liveMatchRoster, opp.roster, opp.label, initialTactics, state.currentMatchNeedsWinner);
  highlightZone('CENTER');
...
function processMatchEvent(ev, options) {
  ...
  if (ev.type === 'penalty' && (ev.outcome === 'missed' || ev.outcome === 'saved') && ev.player) {
    (ev.team === 'me' ? state.livePenaltyMissesMe : state.livePenaltyMissesOpp).push({ player: ev.player, minute: ev.minute, outcome: ev.outcome });
    if (live) renderScoreboard();
  }
