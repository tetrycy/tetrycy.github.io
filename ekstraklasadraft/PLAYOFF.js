...
function autoSimulateRound(round) {
  round.forEach(m => {
    ...
    const r = resolveOtherMatch(m.home, m.away, true, 'neutral'); // drabinka pucharowa - zawsze potrzebny zwycięzca
    m.played = true;
    m.winner = r.result === 'W' ? m.home : m.away;
    m.score = r.wasPenalties
      ? `${r.gf}:${r.ga} (k. ${r.penaltyScore.my}:${r.penaltyScore.opp})`
      : (r.wasExtraTime ? `${r.gf}:${r.ga} (po dogrywce)` : `${r.gf}:${r.ga}`);
    markComputerMatchReplay(m, r.timeline);
    m.scorersHome = r.scorersMe; m.scorersAway = r.scorersOpp;
