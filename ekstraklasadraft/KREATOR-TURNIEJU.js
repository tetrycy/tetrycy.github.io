...
function simulateBackgroundTie(teamA, teamB, legFormat) {
  if (legFormat === 'single-neutral') {
    const r = resolveOtherMatch(teamA, teamB, true, 'neutral'); // pojedynczy mecz - zawsze potrzebny zwycięzca
    const decision = r.wasPenalties
      ? ` (k. ${r.penaltyScore.my}:${r.penaltyScore.opp})`
      : (r.wasExtraTime ? ' (po dogrywce)' : '');
    const scoreText = `${teamA.label} ${r.gf}:${r.ga} ${teamB.label}${decision}`;
    const winner = r.result === 'W' ? teamA : teamB;
    return { winner, scoreText, timeline: r.timeline };
  }
  const r1 = resolveOtherMatch(teamA, teamB, false, 'home');
  const r2 = resolveOtherMatch(teamB, teamA, { myAggOffset: r1.ga, oppAggOffset: r1.gf }, 'home');
  const aggA = r1.gf + r2.ga, aggB = r1.ga + r2.gf;
  const shootoutText = r2.wasPenalties ? `, k. ${r2.penaltyScore.opp}:${r2.penaltyScore.my}` : '';
  const scoreText = `${teamA.label} ${aggA}:${aggB} ${teamB.label} (dwumecz: ${r1.gf}:${r1.ga}, ${r2.ga}:${r2.gf}${shootoutText})`;
  const winner = aggA !== aggB ? (aggA > aggB ? teamA : teamB) : (r2.result === 'W' ? teamB : teamA);
...
function simulateBackgroundGroup(teams, advancePerGroup, legFormat) {
  ...
  fixtures.forEach(round => round.forEach(m => {
    const r = resolveOtherMatch(m.home, m.away, false, 'home'); // faza grupowa - gospodarz dostaje własny bonus
...
function goToMyGroupMatch() {
  ...
  if (!round.__bgSimulated) {
    round.forEach(mt => {
      ...
      const r = resolveOtherMatch(mt.home, mt.away, false, 'home'); // faza grupowa - gospodarz dostaje własny bonus
