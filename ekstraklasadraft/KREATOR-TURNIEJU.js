...
function drawStrongestTeams(n, excludeMineClub) {
  ...
  const withOvr = pool.map(t => {
    ...
    const ovr = usePool.reduce((a, p) => a + p.overall, 0) / usePool.length;
    return { t, ovr };
...
function simulateBackgroundTie(teamA, teamB, legFormat) {
  if (legFormat === 'single-neutral') {
    const r = simulateMatch(teamA.overall, teamB.overall);
    const scoreText = `${teamA.label} ${r.gf}:${r.ga} ${teamB.label}`;
    const winner = r.gf === r.ga ? (Math.random() < 0.5 ? teamA : teamB) : (r.gf > r.ga ? teamA : teamB);
...
function finishPucharKnockoutMatch(sim, m, opp) {
  ...
  if (cp.leg === 1) {
    ...
    document.getElementById('btn-next-round').onclick = () => { showPucharKnockoutMatchInfo(); };
    return;
  }
  ...
  const myAwayGoals = cp.leg1WasHome ? sim.gf : cp.leg1MyGoals;
  const oppAwayGoals = cp.leg1WasHome ? cp.leg1OppGoals : sim.ga;
  if (myAwayGoals !== oppAwayGoals) {
    const timelineEl = document.getElementById('po-timeline');
    const line = document.createElement('div');
    line.className = 'tl-row tl-marker';
    line.textContent = `Remis na dwumeczu ${cp.myAgg}:${cp.oppAgg} — decydują bramki na wyjeździe (${myAwayGoals}:${oppAwayGoals}).`;
    timelineEl.prepend(line); timelineEl.scrollTop = 0;
...
function resolvePucharPairResult(advanced, finalScoreText) {
  ...
  document.getElementById('btn-next-round').style.display = 'inline-block';
  document.getElementById('btn-next-round').onclick = () => { advancePucharRound(); };
}
...
function finishPucharGroupMatch(sim, m, opp) {
  ...
  myGroup.matchIdx++;
  document.getElementById('btn-next-round').textContent = myGroup.matchIdx < myGroup.fixtures.length ? 'NASTĘPNY MECZ GRUPOWY →' : 'ZOBACZ WYNIK GRUPY →';
  document.getElementById('btn-next-round').style.display = 'inline-block';
  document.getElementById('btn-next-round').onclick = () => { goToMyGroupMatch(); };
}
