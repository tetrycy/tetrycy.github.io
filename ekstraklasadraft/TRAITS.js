...
function applyCoachLineBoosts(base, traits) {
  traits.forEach(t => {
    if (t.type === 'lineBoost' && base[t.line] != null) {
      base[t.line] = clamp(base[t.line] + t.amount, 1, 99);
    }
  });
  return base;
}
...
// ── POWTÓRZ MECZ ────────────────────────────────────────────
// Owija ostateczną (w pełni złożoną, po wszystkich trybach gry) wersję
// finishMatch(). Dopóki gracz nie kliknie "ZATWIERDŹ WYNIK", żaden tryb
// gry (playoff/mundial/sezon/wyzwanie/kreator turnieju) nie widzi wyniku
// meczu — nic nie trafia do tabeli/drabinki. Powtórzyć można TYLKO RAZ
// na mecz (state.replayUsedForThisMatch, resetowane w resetMatchScreen
// na starcie każdego nowego meczu).
(function wrapFinishMatchForReplayTrait() {
  ...
  finishMatch = function (sim, m, opp) {
    if (myCoachHasTrait('replayMatch') && !state.replayDecisionMade) {
      ...
      // Tylko JEDNO powtórzenie na mecz — po jego wykorzystaniu drugie
      // zakończenie pokazuje wyłącznie ZATWIERDŹ WYNIK, bez opcji powtórki.
      if (replayBtn) replayBtn.style.display = state.replayUsedForThisMatch ? 'none' : 'inline-block';
      return;
    }
