...
function goToMundial26Setup() {
  if (!Object.keys(state.squad || {}).length) {
    alert('Mundial 2026 grasz swoim wydraftowanym składem — najpierw stwórz drużynę (NOWA GRA), a potem wróć tutaj.');
    goToDraftSetup();
    return;
  }
...
function simulateM26MatchdayBackground(excludeMatch) {
  ...
  Object.keys(m26.groupFixtures).forEach(letter => {
    ...
    md.forEach(m => {
      ...
      const r = resolveOtherMatch(m.home, m.away, false, 'neutral');
...
// Rozstrzyga JEDNĄ grę w tle pełnym silnikiem, łącznie z dogrywką i karnymi.
function m26ResolveBackgroundGame(m) {
  const r = resolveOtherMatch(m.teamA, m.teamB, true, 'neutral');
  return r.result === 'W';
}
...
function setupM26ClassificationMatch() {
  ...
  let venue = 'neutral';
  if (m26.nbaStyleKnockout) {
    const venueSide = NBA_HOME_PATTERN[m.gameIdx];
    venue = venueSide === (iAmA ? 'A' : 'B') ? 'home' : 'away';
