...
function finishTwoLeggedRoundMatch(sim, m, opp, round, onDecided) {
  ...
  // a jeśli dalej remis → rzuty karne. Dogrywka jest symulowana (nie grana
  // na żywo event-po-evencie) — 30 minut to zbyt mało, żeby sensownie
  // uruchamiać pełny generator meczowy na nowo; ważne jest tylko, kto
  // strzeli więcej goli w tym krótkim, dodatkowym okienku.
  const etMy = simulateExtraTimeResult(ch.currentMyTeam.overall, opp.overall);
  ch.myAgg += etMy.gf;
  ch.oppAgg += etMy.ga;
  const timelineEl = document.getElementById('po-timeline');
  const etLine = document.createElement('div');
  etLine.className = 'tl-row tl-marker';
  etLine.textContent = etMy.gf || etMy.ga
    ? `⏱ DOGRYWKA: ${etMy.gf} : ${etMy.ga} — nowy wynik dwumeczu: ${ch.myAgg} : ${ch.oppAgg}.`
    : `⏱ DOGRYWKA: bez goli (0:0) — wynik dwumeczu bez zmian: ${ch.myAgg} : ${ch.oppAgg}.`;
  timelineEl.prepend(etLine);
  timelineEl.scrollTop = 0;
...
// Szybka symulacja 30 minut dogrywki (bez pełnego, event-po-evencie generatora
// meczowego) — niezależna szansa gola dla każdej strony, ważona różnicą siły.
// Najczęstszy wynik to 0:0 (dogrywki rzadko obfitują w bramki).
function simulateExtraTimeResult(myOvr, oppOvr) {
  const diff = myOvr - oppOvr;
  const myGoalChance = clamp(0.30 + diff * 0.01, 0.10, 0.55);
  const oppGoalChance = clamp(0.30 - diff * 0.01, 0.10, 0.55);
  const gf = Math.random() < myGoalChance ? (Math.random() < 0.25 ? 2 : 1) : 0;
  const ga = Math.random() < oppGoalChance ? (Math.random() < 0.25 ? 2 : 1) : 0;
  return { gf, ga };
}
...
function resolveQualifyingResult(def, advanced) {
  ...
  if (advanced) {
    ...
    document.getElementById('challenge-btn-next-round').textContent = 'DALEJ →';
    document.getElementById('challenge-btn-next-round').style.display = 'inline-block';
    document.getElementById('challenge-btn-next-round').onclick = () => { goToChallengeRound(); };
  } else {
...
function initSwissLeague(def) {
  ...
  if (def.swiss.pots) {
    ...
    const pots = def.swiss.pots.map(pot => pot.map(spec => {
...
function computeMyDrawOrder(def) {
  ...
  const myLabel = swissMyLeagueLabel();
  const myFixtures = ch.swissRounds.flat().filter(f => f.home.label === myLabel || f.away.label === myLabel);
  return ch.swissPots.flatMap(pot => myFixtures.filter(f => {
    const oppLabel = f.home.label === myLabel ? f.away.label : f.home.label;
    return pot.includes(oppLabel);
  }));
}
...
// drugim (przycisk "🎲 LOSUJ RYWALA"), koszyk po koszyku, żeby było czuć
...
function goToSwissDrawScreen(def) {
  ...
  document.getElementById('challenge-opponent-preview').textContent = '🎟 Losowanie fazy ligowej — poznaj swoich rywali';
  document.getElementById('challenge-agg-info').textContent = '';
  document.getElementById('challenge-group-table').innerHTML = '';
...
// Renderuje 4 koszyki (wszyscy członkowie) + listę już wylosowanych rywali,
// z krótkim "podbiciem" przy właśnie odsłoniętym (dla dramaturgii).
function renderSwissDrawScreen(justRevealedIdx) {
  ...
  const potsHtml = ch.swissPots.map((pot, i) => {
    const membersHtml = pot.map(label => {
      const isMe = label === myLabel;
      return `<span style="${isMe ? 'color:var(--gold);font-weight:bold;' : ''}">${label}${isMe ? ' (Ty)' : ''}</span>`;
    }).join(' • ');
    return `<div style="margin-bottom:4px;"><b>Koszyk ${i + 1}:</b> ${membersHtml}</div>`;
  }).join('');
  ...
  const revealed = ch.swissMyDrawOrder.slice(0, ch.drawRevealCount).map((f, idx) => {
    ...
    return `<div style="${isNew ? 'color:var(--gold);font-weight:bold;' : ''}">${idx + 1}. ${oppLabel} (${iAmHome ? 'u siebie' : 'wyjazd'})</div>`;
  }).join('') || '<div style="color:var(--gray);">— jeszcze nikogo nie wylosowano —</div>';

  el.innerHTML = `
    <div class="panel-title" style="font-size:14px;">KOSZYKI</div>
    ${potsHtml}
    <div class="panel-title" style="font-size:14px;margin-top:10px;">TWOI RYWALE (${ch.drawRevealCount}/${ch.swissMyDrawOrder.length})</div>
    ${revealed}`;
}

// Losuje (odsłania) kolejnego rywala z ustalonej wcześniej kolejności —
// z krótkim opóźnieniem "Losowanie..." dla odrobiny napięcia.
...
function finishSwissLeagueMatch(sim, m, opp, def) {
  ...
  round.forEach(mt => {
    ...
    const r = simulateMatch(mt.home.overall, mt.away.overall);
  ...
  ch.swissMatchIdx++;
  const isLast = ch.swissMatchIdx >= ch.swissRounds.length;
  document.getElementById('challenge-btn-next-round').textContent = isLast ? 'ZOBACZ KOŃCOWĄ TABELĘ →' : 'NASTĘPNA KOLEJKA →';
  document.getElementById('challenge-btn-next-round').style.display = 'inline-block';
  document.getElementById('challenge-btn-next-round').onclick = () => { goToChallengeRound(); };
}
...
function simBgTie(teamA, teamB) {
  const l1 = simulateMatch(teamA.overall, teamB.overall);
  const l2 = simulateMatch(teamB.overall, teamA.overall);
  const aGoals = l1.gf + l2.ga, bGoals = l1.ga + l2.gf;
  if (aGoals !== bGoals) return aGoals > bGoals ? teamA.label : teamB.label;
  // Remis na agregacie — rozstrzygnięcie losowe, delikatnie ważone różnicą siły.
  return Math.random() < clamp(0.5 + (teamA.overall - teamB.overall) * 0.015, 0.15, 0.85) ? teamA.label : teamB.label;
}
...
function resolveSwissQualifying(def, advanced) {
  ...
  document.getElementById('challenge-btn-next-round').textContent = nextQ ? 'NASTĘPNA RUNDA →' : 'LOSOWANIE →';
  document.getElementById('challenge-btn-next-round').style.display = 'inline-block';
  document.getElementById('challenge-btn-next-round').onclick = () => { goToChallengeRound(); };
}
