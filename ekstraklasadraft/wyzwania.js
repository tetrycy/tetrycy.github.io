...
function resolveOpponentTeam(opponentSpec) {
  ...
  if (opponentSpec.source === 'custom') {
    ...
    return { id: t.id, label: t.club, club: t.club, overall: ovr, isPlayer: false, roster };
  }
  return null;
}
...
function finishTwoLeggedRoundMatch(sim, m, opp, round, onDecided) {
  ...
  if (!sim.wasExtraTime) {
    // Stara ścieżka — zachowana na wypadek, gdyby needsWinner nie było ustawione.
...
function finishSwissLeagueMatch(sim, m, opp, def) {
  ...
  round.forEach(mt => {
    ...
    const r = resolveOtherMatch(mt.home, mt.away, false); // faza ligowa - remis dozwolony
...
function simBgTie(teamA, teamB) {
  const l1 = resolveOtherMatch(teamA, teamB, false); // pierwszy mecz - remis dozwolony
  const l2 = resolveOtherMatch(teamB, teamA, { myAggOffset: l1.ga, oppAggOffset: l1.gf }); // drugi mecz - liczy się zremisowany agregat
  const teamAaggGoals = l1.gf + l2.ga, teamBaggGoals = l1.ga + l2.gf;
  return teamAaggGoals >= teamBaggGoals ? teamA.label : teamB.label;
}
