...
// JEDYNE miejsce, które buduje roster do meczu z jedenastki — używane przez
// WSZYSTKIE tryby (Sezon, Playoff, Mundial, Wyzwania, Kreator Turnieju...).
// Dodanie nowego pola zawodnikowi (np. cechy) wymaga zmiany TYLKO tutaj.
function buildMyMatchRoster() {
  return Object.values(getStartingXISquad()).map(s => ({
    name: s.name, pos: s.pos, overall: s.overall,
    birthYear: s.birthYear, season: s.season, traits: s.traits || [],
  }));
}

// To samo, ale dla JEDNEGO zawodnika z id slotu (żywa lista na ekranie
// meczu, wejście zmiennika) — ten sam zestaw pól, żeby nie rozjeżdżało się
// z buildMyMatchRoster() powyżej.
function buildMatchRosterEntry(p, slotId) {
  return {
    name: p.name, pos: p.pos, overall: p.overall,
    birthYear: p.birthYear, season: p.season, traits: p.traits || [],
    __slotId: slotId,
  };
}
...
function goToManageSquad() {
  ...
  switchSquadTab(state.squadActiveTab || 'overview');
}
...
// ── PROFIL ZAWODNIKA — patrz PROFILE.js ────────────────────
...
function renderManageSquad() {
  ...
  activeSorted.forEach(id => {
    ...
    const nameSpan = row.querySelector('.lineup-name');
    if (nameSpan && !pendingReservePos) {
      nameSpan.style.cursor = 'pointer';
      nameSpan.style.textDecoration = 'underline';
      nameSpan.addEventListener('click', e => { e.stopPropagation(); openPlayerProfile(id); });
    }
