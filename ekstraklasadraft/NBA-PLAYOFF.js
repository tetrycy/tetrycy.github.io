...
function resolveNonPlayerSeriesInBackground() {
  state.nbaPlayoff.series.forEach(s => {
    ...
    while (!s.winner) {
      const r = resolveOtherMatch(s.teamA, s.teamB, true); // każda gra serii potrzebuje zwycięzcy
...
function decideNbaGameWinner(sim, myTeam, opp) {
  ...
  if (sim.wasExtraTime) {
    // Dogrywka już się ROZEGRAŁA NA ŻYWO (w samym silniku, patrz needsWinner
    // w simulateMatchV2Live) i mecz WCIĄŻ jest remisowy — od razu karne, bez
    // dodatkowego (już zbędnego) cichego rozstrzygnięcia.
  ...
  if (et.gf !== et.ga) return { myWon: et.gf > et.ga, note: ` (dogr. ${et.gf}:${et.ga})` };
  const shootout = simulatePenaltyShootout(myTeam.roster, opp.roster, opp.label);
  return { myWon: shootout.iWin, note: ` (dogr. ${et.gf}:${et.ga}, k. ${shootout.myScore}:${shootout.oppScore})` };
}
