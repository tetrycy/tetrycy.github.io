...
function resolveNonPlayerSeriesInBackground() {
  state.nbaPlayoff.series.forEach(s => {
    ...
    while (!s.winner) {
      const r = simulateMatch(s.teamA.overall, s.teamB.overall);
      if (r.gf > r.ga) s.winsA++;
      else if (r.ga > r.gf) s.winsB++;
      else if (Math.random() < 0.5) s.winsA++; // remis w tle rozstrzygamy monetą (bez pełnej dogrywki/karnych)
      else s.winsB++;
...
function finishNbaPlayoffMatch(sim, m, opp) {
  ...
  const scoreText = `${sim.gf} : ${sim.ga}${decision.note}`;
