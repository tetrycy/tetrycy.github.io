...
// ============================================================
// UPROSZCZONY MECZ (mecze w tle — których gracz nie ogląda: inne
// pary w turnieju, inne mecze kolejki sezonu, itp.). To NIE jest
// pełny silnik V2 z timeline'em zdarzeń — tylko szybki wynik na
// podstawie różnicy overallu, do rozstrzygania wielu meczów naraz.
// ============================================================
function simulateMatch(myOvr, oppOvr) {
  const diff = myOvr - oppOvr;
  // Krzywa zwycięstwa/przegranej - stroma, żeby duża różnica overallu
  // faktycznie robiła różnicę (wcześniej: sufit szansy faworyta to zawsze
  // 64%, niezależnie od tego, czy przewaga to 20 punktów, czy 37 - stąd
  // dużo za częste niespodzianki w meczach w tle).
  const winRaw = clamp(0.5 + diff * 0.022, 0.03, 0.94);
  const lossRaw = clamp(0.5 - diff * 0.022, 0.03, 0.94);
  // Remis: najczęstszy przy wyrównanych siłach (realistyczne ~27%), coraz
  // rzadszy im większa przepaść — wcześniej był sztywnym 16% zawsze,
  // niezależnie od różnicy sił.
  const drawChance = clamp(0.27 - Math.abs(diff) * 0.006, 0.05, 0.27);
  const remaining = 1 - drawChance;
  const winChance = (winRaw / (winRaw + lossRaw)) * remaining;
  const roll = Math.random();
  let result = roll < winChance ? 'W' : roll < winChance + drawChance ? 'D' : 'L';
...
function simulateMatchFullEngine(myRoster, oppRoster, oppLabel, needsWinner) {
  const gen = simulateMatchV2Live(myRoster, oppRoster, oppLabel, {}, needsWinner);
  let r = gen.next();
  ...
  return { result: final.result, gf: final.gf, ga: final.ga, timeline: final.timeline, scorersMe, scorersOpp, wasExtraTime: final.wasExtraTime };
}

// ── WSPÓLNY DYSPOZYTOR dla meczów w tle ──────────────────────────
// Wybiera silnik wg ustawień gracza (state.settings.otherMatchesEngine).
// ZAWSZE zwraca ten sam kształt {result, gf, ga, timeline, scorersMe,
// scorersOpp} — timeline/scorersMe/scorersOpp są null w trybie szybkim,
// wypełnione w trybie pełnym. Dzięki temu wywołujący kod NIE MUSI wiedzieć,
// który silnik akurat pracował — po prostu sprawdza, czy timeline istnieje.
//
// teamA/teamB: obiekty drużyn z .overall i (jeśli dostępny) .roster.
...
function resolveOtherMatch(teamA, teamB, needsWinner) {
  const wantsFull = state.settings && state.settings.otherMatchesEngine === 'full';
  if (wantsFull && teamA.roster && teamA.roster.length && teamB.roster && teamB.roster.length) {
    return simulateMatchFullEngine(teamA.roster, teamB.roster, teamB.label, needsWinner);
  }
  let r = simulateMatch(teamA.overall, teamB.overall);
  ...
  const aggregateTied = !!needsWinner && (r.gf + aggOffsetMy) === (r.ga + aggOffsetOpp);
  if (aggregateTied) {
    wasExtraTime = true;
    // Tryb szybki: remis (na AGREGACIE, nie tylko tym meczu), gdy potrzebny
    // jest zwycięzca, rozstrzyga się też szybko (dogrywka jako rzut kostką,
    // potem moneta) — to wciąż tło, którego nikt nie ogląda.
    const et = simulateExtraTimeResult(teamA.overall, teamB.overall);
    if (et.gf !== et.ga) { r = { result: et.gf > et.ga ? 'W' : 'L', gf: r.gf, ga: r.ga }; }
    else {
      const aWon = Math.random() < clamp(0.5 + (teamA.overall - teamB.overall) * 0.015, 0.15, 0.85);
      r = { result: aWon ? 'W' : 'L', gf: r.gf, ga: r.ga };
    }
  }
  return { result: r.result, gf: r.gf, ga: r.ga, timeline: null, scorersMe: null, scorersOpp: null, wasExtraTime };
}

// Krótki, czytelny tekst strzelców do dopisania przy wyniku — "Kowalski 34',
// Nowak 67'" — albo pusty string, jeśli nie ma danych (tryb szybki).
...
function* simulateMatchV2Live(myRoster, oppRoster, oppLabel, initialTactics, needsWinner) {
  ...
  // TRAITS trenerskie (TRAITS.js) — na razie tylko trener GRACZA (state.coach);
  // AI/rywale nie mają jeszcze podpiętych swoich trenerów do silnika.
  const myCoachTraits = (typeof getMyCoachTraits === 'function') ? getMyCoachTraits() : [];
    ...
    if (!forceAction && r1 < flavorShare) {
      ...
      if (r2 < 8) {
        ...
        if (c >= 2 || provokedRed) {
          ...
          const provokedTemplate = provokedRed && c < 2 ? ST.redOpp : (ST.secondYellowOpp || ST.redOpp);
        ...
        const meAtFault = Math.random() < clamp(0.5 - myOppRedBonus, 0.05, 0.95);
        ...
        const meAwarded = Math.random() < clamp(0.5 + myPenaltyAwardBonus, 0.05, 0.95);
