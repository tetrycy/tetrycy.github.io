  ...
  Object.keys(m26.groupFixtures).forEach(letter => {
    ...
    md.forEach(m => {
      ...
      m26.groupResults[letter].push({
        ...
        timeline: r.timeline, scorersHome: r.scorersMe, scorersAway: r.scorersOpp,
      });
    });
  });
}
...
function m26ResolveBackgroundGame(m) {
  const r = resolveOtherMatch(m.teamA, m.teamB, true); // pojedyncza gra zawsze potrzebuje zwycięzcy
  return { aWon: r.result === 'W', gf: r.gf, ga: r.ga, timeline: r.timeline, scorersHome: r.scorersMe, scorersAway: r.scorersOpp };
}
...
function m26ResolveBackgroundMatch(m) {
  ...
  if (!m26.nbaStyleKnockout) {
    ...
    m.scoreText = `${g.gf}:${g.ga}`;
    m.timeline = g.timeline; m.scorersHome = g.scorersHome; m.scorersAway = g.scorersAway;
    return;
  }
  while (!m.winner) {
    const g = m26ResolveBackgroundGame(m);
...
function m26RegisterGameResult(match, sim, myTeam, opp, iAmA) {
  ...
  if (!m26.nbaStyleKnockout) {
    ...
    if (sim.result === 'D') {
      if (sim.wasExtraTime) {
        // Dogrywka już się ROZEGRAŁA NA ŻYWO (patrz needsWinner w
        // simulateMatchV2Live) i mecz WCIĄŻ jest remisowy — od razu karne.
        // Jeśli seria już została odsłonięta na ekranie (m26DecideDrawEarly),
        // używamy DOKŁADNIE TEJ SAMEJ — nie losujemy drugi raz.
