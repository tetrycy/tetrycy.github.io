...
function applyFogOfWarIfNeeded() {
  ...
  Object.keys(state.squad).forEach(id => {
    ...
    p.overall = clamp(p.overall + randIntRange(5), 1, 99);
  });
...
function applyTrainingDeltasToRoster(roster) {
  ...
  return roster.map(p => {
    ...
    if (!delta) return p;
    return Object.assign({}, p, { overall: clamp(p.overall + delta, 1, 99) });
  });
}
...
function applyPermanentTrainingTick() {
  ...
  Object.keys(state.squad).forEach(id => {
    ...
    const p = state.squad[id];
    const delta = randIntRange(2);
    if (!delta) return;
    p.overall = clamp(p.overall + delta, 1, 99);
