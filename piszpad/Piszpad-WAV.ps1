...
$port = 8765
$htmlPath = Join-Path $PSScriptRoot 'Piszpad.html'
...
if (-not (Test-Path -LiteralPath $htmlPath)) {
    Write-Host 'Brak pliku Piszpad.html obok generatora.' -ForegroundColor Red
    ...
    while ($true) {
        ...
        try {
            ...
            if ($request.Method -eq 'GET' -and
                ($request.Path -eq '/' -or $request.Path -eq '/Piszpad.html')) {
