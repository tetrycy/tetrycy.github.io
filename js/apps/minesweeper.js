...
class Minesweeper {
    constructor() {
        ...
        // Większe poziomy trudności - dopasowane do okna
        this.difficulties = {
            beginner: { width: 12, height: 10, mines: 15 },
            intermediate: { width: 16, height: 13, mines: 35 },
            expert: { width: 20, height: 16, mines: 65 }
        };
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', () => {
            const contextMenu = document.getElementById('mine-context-menu');
            if (contextMenu) {
    ...
    newGame(difficulty = 'beginner') {
        ...
        // Dostosuj rozmiar okna do planszy
        this.resizeWindow(config.width, config.height);
    }
    ...
    resizeWindow(width, height) {
        ...
        if (window) {
            ...
            const boardWidth = width * cellSize + padding;
            const boardHeight = height * cellSize + padding;
            const totalHeight = headerHeight + menuHeight + infoHeight + boardHeight + 20;
            
            window.style.width = Math.max(300, boardWidth + 20) + 'px';
            window.style.height = Math.max(250, totalHeight) + 'px';
        }
    }
